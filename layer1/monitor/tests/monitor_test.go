...
func TestMonitorPersist(t *testing.T) {
	...
	mon, err := monitor.NewMonitor(database, database, mocks.NewMockAdminHandler(), mocks.NewMockDepositHandler(), eth, 1*time.Second, 100, make(chan tasks.TaskRequest, 10))
	assert.Nil(t, err)
	...
	err = mon.State.PersistState(database)
	assert.Nil(t, err)

	//
	newMon, err := monitor.NewMonitor(database, database, mocks.NewMockAdminHandler(), mocks.NewMockDepositHandler(), eth, 1*time.Second, 100, make(chan tasks.TaskRequest, 10))
	assert.Nil(t, err)

	err = newMon.State.LoadState(database)
	assert.Nil(t, err)
...
func TestBidirectionalMarshaling(t *testing.T) {
	...
	// Setup monitor state
	mon, err := monitor.monitor.NewMonitor(&db.Database{}, &db.Database{}, adminHandler, depositHandler, eth, 2*time.Second, time.Minute, 1, make(chan interfaces.ITask, 10), make(chan string, 10))
	assert.Nil(t, err)
	populateMonitor(mon.State, EPOCH)
	...
	// Unmarshal
	newMon, err := monitor.monitor.NewMonitor(&db.Database{}, &db.Database{}, adminHandler, depositHandler, eth, 2*time.Second, time.Minute, 1, make(chan interfaces.ITask, 10), make(chan string, 10))
	assert.Nil(t, err)
